...
#include <fcntl.h>
#else
#include "../../../../codingground/Prioridade/estruturas/xlink.h"
#endif
...
//  huff_initVitter(alphabet_size, potential symbols used)
//  huff_encodeVitter(next_symbol)
...
HCoder *huff_initVitter (unsigned size, unsigned root)
{
HCoder *huff;
...
//      huff->map = zalloc (size * sizeof(unsigned));
  huff->map = malloc (size * sizeof(unsigned));
#endif
...
unsigned empty = 0, max;
    ...
    if( max = huff->size - (huff->root - huff->esc) / 2 - 1 )
      do arc_put1Unsigned (empty & 1), empty >>= 1;
      while( max >>= 1 );
}

//  encode the next symbol

void huff_encodeVitter (HCoder *huff, unsigned symbol)
{
unsigned emit = 1, bit;
...
unsigned empty = 0, bit = 1, max, symbol;
    ...
    if( max = huff->size - (huff->root - huff->esc) / 2 - 1 )
      do empty |= arc_get1Unsineg () ? bit : 0, bit <<= 1;
      while( max >>= 1 );
...
int mainVitter (int argc, char **argv)
{
...
Out = stdout;
    ...
    while( size )
      if( symbol = getc(In), huff_encodeVitter(huff, symbol), size-- & mask )
        continue;
      else
        huff_scale(huff, 1);
...
void arc_put1Unsigned (unsigned bit)
{
    ArcChar <<= 1;
