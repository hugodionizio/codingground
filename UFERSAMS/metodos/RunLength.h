...
/* Specification: encodeRLEd stream are unsigned bytes consisting of sequences.
 ...
 * This scheme can encodeRLE arbitrary byte values efficiently.
 * c.f. Adobe PDF spec RLE stream encoding (not exactly the same)
 */
void encodeRLE(stream in, stream out)
{
...
void decodeRLE(stream in, stream out)
{
	int c, i, cnt;
	while (1) {
		c = in->get(in);
...
int testRunLength(char *input)
{
	char buf[256];
	string_stream str_in = { sget, 0, input, 0};
	string_stream str_out = { sget, sput, buf, 0 };
	file_stream file = { 0, file_put, stdout };

	/* encodeRLE from str_in to str_out */
	encodeRLE((stream)&str_in, (stream)&str_out);

	/* decodeRLE from str_out to file (stdout) */
	decodeRLE((stream)&str_out, (stream)&file);
