...
unsigned char prev;
    ...
    for( idx = 0; idx < HUFF_size; idx++ ) {
      for( length[idx] = bits = 0; bits < 5; bits++ )
        length[idx] <<= 1, length[idx] |= arc_get1Int();
      rank[length[idx]]++;
    }
...
KeyPrefix *keys;
    ...
    arc_put8Int ((unsigned char)(keys->prefix >> 16));
    arc_put8Int ((unsigned char)(keys->prefix >> 8));
    arc_put8Int ((unsigned char)(keys->prefix));
...
int mainBWTCoder (int argc, char **argv)
{
...
void arc_put8Int (int ch)
{
int idx = 8;

    while( idx-- )
        arc_put1Int (ch & 1 << idx);
}
